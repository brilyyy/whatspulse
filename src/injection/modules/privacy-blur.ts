...
import type { WhatsPulsePrivacyConfig } from "../types";

const STYLE_ID = "whatspulse-privacy-blur";
const OVERLAY_ID = "whatspulse-idle-overlay";

let currentConfig: WhatsPulsePrivacyConfig | null = null;
...
function getRadii(cfg?: WhatsPulsePrivacyConfig | null): Radii {
  const base = cfg && typeof cfg.blur_radius === "number" && cfg.blur_radius > 0 ? cfg.blur_radius : 8;
...
function buildCss(cfg?: WhatsPulsePrivacyConfig | null): string {
  if (!cfg || cfg.enabled === false) {
    return "";
  }

  const r = getRadii(cfg);
  const noDelay = cfg.no_transition_delay === true;
  const hoverDelay = noDelay ? "0.04s !important" : "0.1s";
  const hoverDuration = noDelay ? "0.04s !important" : "0.15s";
  ...
  const hoverTrans = "transition-delay: " + hoverDelay + "; transition-duration: " + hoverDuration + ";";
  ...
  if (cfg.blur_media !== false) {
    css.push(
      ...
      'div[data-testid="msg-container"] div > button:has(>img),',
      ...
      'div[data-testid="msg-container"] div > button:has(>img):hover,',
...
function removeIdleOverlay(): void {
...
function showIdleOverlay(): void {
  ...
  if (!existing) {
    ...
    overlay.style.cssText =
      "position:fixed;top:0;left:0;right:0;bottom:0;backdrop-filter:blur(var(--wi-blur, 14px));-webkit-backdrop-filter:blur(var(--wi-blur, 14px));background:rgba(10,14,19,0.78);z-index:9999999;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;user-select:none;animation:wpFadeIn .2s ease;";
    overlay.innerHTML =
      '<div style="background:#131b24;border:1px solid rgba(0,230,153,0.3);box-shadow:0 12px 40px rgba(0,0,0,0.6);border-radius:18px;padding:32px 40px;text-align:center;max-width:380px;">' +
...
function resetIdleTimer(): void {
  ...
  if (currentConfig && currentConfig.enabled !== false && currentConfig.blur_on_idle === true) {
    const timeoutMs = (currentConfig.idle_timeout_seconds || 120) * 1000;
...
export function applyPrivacy(cfg?: WhatsPulsePrivacyConfig | null): void {
  try {
    currentConfig = cfg || {};
    ...
    reportScriptError("privacy-blur", e);
  }
}

export function initPrivacyBlur(): void {
  window.__whatspulseSetPrivacy = applyPrivacy;

  window.__whatspulseSetBlur = (level: number) => {
    if (!currentConfig) currentConfig = {};
    currentConfig.enabled = level > 0;
    currentConfig.blur_radius = level <= 0 ? 0 : level === 1 ? 4 : level === 2 ? 8 : 14;
    applyPrivacy(currentConfig);
  };

  setupIdleListeners();

  let initial: WhatsPulsePrivacyConfig | null = window.__whatspulseConfig?.privacy || null;
  if (!initial && window.__whatspulseConfig) {
    const lvl = window.__whatspulseConfig.blurLevel || 0;
    initial = {
      enabled: lvl > 0,
