...
//           QWebChannel bridge.
// depends:  window.__whatspulse.bridge.openSettings; the rail's <button> entries and
...
(function () {
    'use strict';
    var ID = 'whatspulse-settings-nav';
    var DIRECT_ID = 'whatspulse-direct-chat-nav';
    var DIRECT_ICON_VIEWBOX = '0 0 24 24';
    var DIRECT_ICON = '<path fill="currentColor" d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-2 12H6v-2h12v2zm0-3H6V9h12v2zm0-3H6V6h12v2z"/>';
    // The WhatsPulse mark (speech bubble + W), viewBox 0 0 64 64, filled with
    // currentColor so it inherits the rail's icon colour. Sourced from
    // resources/icons/whatspulse-symbolic.svg.
    ...
    function anchors() {
        ...
        var mine = function (b) { return b.closest('[id^="whatspulse-"]'); };
        var avatar = null;
    ...
    function install() {
        try {
            var a = anchors();
            if (!a) { return; }
            if (placed(a) && document.getElementById(DIRECT_ID)) { return; }

            var existing = document.getElementById(ID);
            if (existing) { existing.remove(); }
            var existingDirect = document.getElementById(DIRECT_ID);
            if (existingDirect) { existingDirect.remove(); }
            ...
            // 2. Direct Chat entry
            var directEntry = templateWrapper.cloneNode(true);
            directEntry.id = DIRECT_ID;
            Array.prototype.slice.call(directEntry.querySelectorAll('[data-testid]'))
                .forEach(function (n) { n.removeAttribute('data-testid'); });
            var directBtn = directEntry.querySelector('button') || directEntry;
            ...
            var directSvg = directBtn.querySelector('svg');
            if (directSvg) {
    ...
    var INPAGE_MODAL_ID = 'whatspulse-direct-chat-modal';

    function showInpageDirectChat() {
        var existing = document.getElementById(INPAGE_MODAL_ID);
        if (existing) {
            existing.remove();
        }
        ...
        var bgBackdrop = 'rgba(0, 0, 0, 0.72)';
        var bgCard = isDark ? '#111b21' : '#ffffff';
        var textPrimary = isDark ? '#e9edef' : '#111b21';
        var textSecondary = isDark ? '#8696a0' : '#667781';
        var borderCard = isDark ? 'rgba(134, 150, 160, 0.2)' : 'rgba(0, 0, 0, 0.12)';
        var bgInput = isDark ? '#202c33' : '#f0f2f5';
        var accentColor = '#00a884';

        var overlay = document.createElement('div');
        overlay.id = INPAGE_MODAL_ID;
        overlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;z-index:9999999;background:' + bgBackdrop + ';backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;font-family:system-ui,-apple-system,sans-serif;animation:wpModalFadeIn .15s ease;';

        overlay.innerHTML =
            '<div style="background:' + bgCard + ';border:1px solid ' + borderCard + ';border-radius:16px;box-shadow:0 16px 48px rgba(0,0,0,0.4);width:440px;max-width:92vw;padding:28px;color:' + textPrimary + ';box-sizing:border-box;">' +
                '<div style="display:flex;gap:14px;align-items:flex-start;margin-bottom:20px;">' +
                    '<div style="width:40px;height:40px;border-radius:10px;background:rgba(0,168,132,0.15);color:' + accentColor + ';display:flex;align-items:center;justify-content:center;flex-shrink:0;">' +
                    ...
                    '</div>' +
                    '<div style="flex:1;">' +
                        '<h3 style="margin:0 0 4px;font-size:17px;font-weight:600;color:' + textPrimary + ';">Direct Chat</h3>' +
                        '<p style="margin:0;font-size:12px;color:' + textSecondary + ';line-height:1.4;">Kirim pesan WhatsApp langsung tanpa menyimpan nomor ke kontak.</p>' +
                    '</div>' +
                '</div>' +

                '<div style="margin-bottom:16px;">' +
                    '<label style="display:block;font-size:12px;font-weight:600;margin-bottom:6px;color:' + textPrimary + ';">Nomor Telepon WhatsApp</label>' +
                    '<input type="tel" id="wp-inpage-phone" placeholder="Contoh: 08123456789 atau +628123456789" style="width:100%;padding:10px 14px;border-radius:8px;border:1px solid ' + borderCard + ';background:' + bgInput + ';color:' + textPrimary + ';font-size:14px;box-sizing:border-box;outline:none;" />' +
                    '<span style="display:block;font-size:11px;color:' + textSecondary + ';margin-top:4px;">Format lokal 08... otomatis dikonversi ke kode negara 628...</span>' +
                '</div>' +

                '<div style="margin-bottom:20px;">' +
                    '<label style="display:block;font-size:12px;font-weight:600;margin-bottom:6px;color:' + textPrimary + ';">Pesan Pembuka (Opsional)</label>' +
                    '<textarea id="wp-inpage-msg" rows="3" placeholder="Tuliskan pesan pembuka di sini..." style="width:100%;padding:10px 14px;border-radius:8px;border:1px solid ' + borderCard + ';background:' + bgInput + ';color:' + textPrimary + ';font-size:14px;box-sizing:border-box;outline:none;resize:vertical;font-family:inherit;"></textarea>' +
                '</div>' +
                ...
                '<div style="display:flex;justify-content:flex-end;gap:10px;">' +
                    '<button id="wp-inpage-cancel" style="padding:8px 16px;border-radius:8px;border:none;background:transparent;color:' + textSecondary + ';font-size:13px;font-weight:500;cursor:pointer;">Batal</button>' +
                    '<button id="wp-inpage-submit" style="padding:8px 18px;border-radius:8px;border:none;background:' + accentColor + ';color:#ffffff;font-size:13px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;">' +
        ...
        function submit() {
            var phoneInput = overlay.querySelector('#wp-inpage-phone');
            var msgInput = overlay.querySelector('#wp-inpage-msg');
            var errBanner = overlay.querySelector('#wp-inpage-error');
            ...
            close();
            var targetUrl = 'https://web.whatsapp.com/send?phone=' + cleaned + (msg ? '&text=' + encodeURIComponent(msg) : '');
        ...
        overlay.addEventListener('keydown', function (e) {
            if (e.key === 'Escape') {
                close();
            } else if (e.key === 'Enter' && (e.target.id === 'wp-inpage-phone' || (e.ctrlKey && e.target.id === 'wp-inpage-msg'))) {
                submit();
            }
        });

        document.body.appendChild(overlay);
        var input = overlay.querySelector('#wp-inpage-phone');
        if (input) {
